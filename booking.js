...
function getSlotStatus(dateStr) {
  // Returns: 'free' | 'half' | 'full'
  const bookings = getBookings();
  const dayBookings = bookings.filter(b => b.date === dateStr && b.status !== 'rejected');
  const morning = dayBookings.some(b => b.slot === 'morning');
  const evening = dayBookings.some(b => b.slot === 'evening');
...
function selectDate(dateStr, cell) {
  ...
  if (mBtn) {
    mBtn.classList.remove('active','disabled');
    if (taken.includes('morning')) mBtn.classList.add('disabled');
  }
  if (eBtn) {
    eBtn.classList.remove('active','disabled');
    if (taken.includes('evening')) eBtn.classList.add('disabled');
  }
}
...
function submitBooking() {
  ...
  if (!selectedDate) {
    showMsg(msgEl, t.form_err_date, 'error'); return;
  }
  if (!selectedSlot) {
    showMsg(msgEl, t.form_err_slot, 'error'); return;
  }
  if (!name || !phone || !event) {
    showMsg(msgEl, t.form_err_fields, 'error'); return;
  }

  const booking = {
    id: Date.now(),
    name, phone, event, slot: selectedSlot,
    date: selectedDate, guests, note,
    status: 'pending',
  ...
  // Reset form
  ['f_name','f_phone','f_guests','f_note'].forEach(id => {
...
function showMsg(el, text, type) {
  ...
  setTimeout(() => { if (type === 'success') el.style.display = 'none'; }, 6000);
}
...
// Re-render calendar when language changes (for month name)
